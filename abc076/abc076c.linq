...
static class Program
{
	static void Main()
	{
		...
		var count = s.Count() - t.Count() + 1;
		
		var ac = "UNRESTORABLE";
		
		foreach (var i in Ie(0, count))
		{
			var ss = s.Substring(i, t.Count());
			...
			if (!fail)
			{
				...
				for (var k = 0; k < s.Count(); k++)
				{
					if (i <= k && k < i + t.Count())
					{
						l.Add(t.ToCharArray()[k-i]);
						continue;
					}
					
					var c = s.ToCharArray()[k];
					if (c.Equals('?'))
					{
						l.Add('a');
					}
