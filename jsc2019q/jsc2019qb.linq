...
static class Program
{
	static void Main()
	{
		...
		var dbl = new List<int>();
		dbl.AddRange(a);
		dbl.AddRange(a);

		long GetBubbleSortCount(int[] ia)
		{
			var res = 0L;
			foreach (var i in Ie(0, ia.Length))
			{
				foreach (var j in Ie(0, ia.Length - 1))
				{
					if (ia[j] > ia[j + 1])
					{
						var f = ia[j];
		...
		var x = GetBubbleSortCount(a);
		var y = GetBubbleSortCount(dbl.ToArray()) - 2 * x;
		
		x = x % M;
		y = y % M;
		
		var result = x * k % M + y * k % M * (k - 1) % M * ModInv(2, M) % M;
