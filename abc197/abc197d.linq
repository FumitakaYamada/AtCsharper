...
", "").Replace("\r", ""))
	...
	static void Main()
	{
		...
		var inputs2 = inputter.GetNext().Split().Select(ToInt).ToArray();
		
		var p0 = new Point(inputs);
		
		var phalf = new Point(inputs2);
		
		var center = new Point((p0.X + phalf.X) / 2f, (p0.Y + phalf.Y) / 2f);

		var theta = Math.PI * 2 / n;
		
		var p1 = RotateCounterClock(center, p0, theta);
