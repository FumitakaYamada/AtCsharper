...
static class Program
{
	static void Main()
	{
		...
		while (ac + 1 < wc)
		{
			...
			if (s.Contains(new string('L', wj)))
			{
				fl = true;
			}
			if (s.Contains(new string('R', wj)))
			{
				fl = true;
			}
		...
		var gm = new Dictionary<int, int>();

		var nl = Ie(0, s.Count()).Select(x => 0).ToArray();
		foreach (var i in Ie(0, s.Count()))
		{
			var f = ca[i];
			var mv = ca[i].Equals('R') ? 1 : -1;
			
			foreach (var j in Ie(1, ac))
			{
				if (j % 2 == 0 && gm.ContainsKey(i+j*mv))
				{
					nl[gm[i + j * mv]]++;

					foreach (var k in Ie(0, j / 2))
					{
						if (gm.ContainsKey(i + mv * k * 2))
						{
							break;
						}
						gm.Add(i + mv * k * 2, gm[i + j * mv]);
					}
					
					break;
				}
				
				if (ca[i+j*mv].Equals(f))
				{
					continue;
				}
	...
	$@"RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL";
