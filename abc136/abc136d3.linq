...
static class Program
{
	static void Main()
	{
		...
		foreach (var c in s.ToCharArray())
		{
			if (c.Equals('R'))
			{
				if (or)
				{
					l.Add(s.Substring(start, i - start));
					start = i;
					or = false;
				}
			}
		...
		l.Add(s.Substring(start, s.Length - start));
		
		var strl = new List<string[]>();

		foreach (var ss in l)
		{
			var rc = ss.ToCharArray().Count(x => x.Equals('R'));
			var lc = ss.ToCharArray().Count(x => x.Equals('L'));
			
			var str = new List<string>();
			
			if (rc > 1)
			{
				str.AddRange(Ie(0, rc - 1).Select(x => "0").ToArray());
			}

			str.Add((Math.Ceiling(rc / 2f) + Math.Floor(lc / 2f)).ToString());
			str.Add((Math.Ceiling(lc / 2f) + Math.Floor(rc / 2f)).ToString());

			if (lc > 1)
			{
				str.AddRange(Ie(0, lc - 1).Select(x => "0").ToArray());
			}

			strl.Add(str.ToArray());
		}
		
		Wl(string.Join(' ', strl.SelectMany(x => x).ToArray()));
	}
	...
	$@"RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLRLRL";
