...
static class Program
{
	static void Main()
	{
		...
		var dic = l.GroupBy(x => x[0]).ToDictionary(x => x.Key, x => x.Select(y => y[1]).ToArray());

		var sum = 0L;

		foreach (var from in Ie(n))
		{
			var queue = new PriorityQueue<int>();
			var went = new bool[2000];
			...
			while (queue.Any())
			{
				...
				if (went[current]) continue;
				
				//("to : " + current).Dump();
				
				went[current] = true;
				
				if (!dic.ContainsKey(current)) continue;
				
				foreach (var dest in dic[current])
				{
					queue.Enqueue(dest);
				}
			}
			
			foreach (var to in Ie(n))
			{
				if (went[to]) sum++;
			}
		}
