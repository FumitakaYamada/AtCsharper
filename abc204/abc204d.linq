...
static class Program
{
	static void Main()
	{
		...
		var tt = inputter.GetNext().Split().Select(ToInt).OrderByDescending(x => x).ToArray();
		
		var dp = new int[101, 1000000];
		
		var sum = tt.Sum();
		var half = (int)Math.Ceiling(sum / 2f);

		foreach (var i in Ie(1, n))
		{
			foreach (var j in Ie(1, half))
			{
				//(i + " " + j).Dump();
				if (j - tt[i - 1] >= 0) dp[i, j] = Math.Max(dp[i - 1, j - tt[i - 1]] + tt[i - 1], dp[i - 1, j]);
				else dp[i, j] = dp[i - 1, j];
			}
		}
		
		Wl(Math.Max(dp[n, half], sum - dp[n, half]));
	}
