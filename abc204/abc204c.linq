...
static class Program
{
	static void Main()
	{
		...
		foreach (var i in Ie(m))
		{
			...
			{
				var arr = dic[from].ToList();
				arr.Add(to);
				dic[from] = arr.ToArray();
			}
			else
			{
				dic.Add(from, new int[] { to });
			}
		}
		...
		int[] dp(int from)
		{
			if (newDic.ContainsKey(from))
			{
				return newDic[from];
			}

			if (!dic.ContainsKey(from)) return new int[] {};
			
			var all = new List<int[]>();
			
			foreach (var to in dic[from])
			{
				all.Add(dp(to));
			}
