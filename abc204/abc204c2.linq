...
static class Program
{
	static void Main()
	{
		...
		var ikeru = new Dictionary<int, List<int>>();
		var koreru = new Dictionary<int, List<int>>();

		foreach (var i in Ie(n))
		{
			ikeru.Add(i, new int[] { i }.ToList());
			koreru.Add(i, new int[] { i }.ToList());
		}
		...
		foreach (var i in Ie(m))
		{
			...
			var korerus = koreru[from];
			
			foreach (var k in korerus)
			{
				//ikeru[k].AddRange(ikerus);
				ikeru[k].Add(to);
			}
			
			foreach (var ii in ikerus)
			{
				//koreru[ii].AddRange(korerus);
				koreru[ii].Add(from);
			}
		}
		...
		foreach (var dest in ikeru)
		{
			var amount = dest.Value.Distinct().Count();
			sum += amount;
		}
