...
", "").Replace("\r", ""))
	...
	static void Main()
	{
		...
		var a1 = inputter.GetNext().Split().Select(ToInt).ToArray();
		var a2 = inputter.GetNext().Split().Select(ToInt).ToArray();
		var a3 = inputter.GetNext().Split().Select(ToInt).ToArray();
		var n = inputter.GetNext().ToInt();
		var b = new List<int>();
		rep(n, i => b.Add(inputter.GetNext().ToInt()));
		...
		Console.WriteLine(cl.Any(x => x.All(y => b.Contains(al[y-1]))) ? "Yes" : "No");
	}
}
