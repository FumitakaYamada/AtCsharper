...
static class Program
{
	static void Main()
	{
		...
		var nl = (new string(stack.ToArray())).Split(',').Where(x => x.Length > 0).ToArray();

		var unk = -1;
		var l = new List<long>();

		var str = string.Join(',', nl.SelectMany(x =>
		{
			var list = new List<int>();
			if (x.First().Equals('<'))
			{
				list.AddRange(Ie(0, x.Count()));
				list.Add(unk);
			}
			else
			{
				list.Add(unk);
				list.AddRange(Ie(0, x.Count()).Reverse());
			}
			return list.ToArray();
		}).Select(x => x.ToString()));
