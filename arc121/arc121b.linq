...
static class Program
{
	static void Main()
	{
		...
		foreach (var i in Ie(n * 2))
		{
			var arr = inputter.GetNext().Split().ToArray();
		...
		var amariColor = l.GroupBy(x => x[1]).Where(x => x.Count() % 2 == 1).Select(x => x.Key).ToArray();

		if (amariColor.Count() == 0)
		{
			Wl(0);
			return;
		}
		
		if (amariColor.Count() != 2)
		{
			throw new Exception();
		}
		
		var lastSmaller = l.Where(x => x[1] == amariColor[0]).Select(x => x[0]).Min();
		var lastBigger = l.Where(x => x[1] == amariColor[1]).Select(x => x[0]).Min();
		
		var otherExist = l.Any(x => x[1] != amariColor[0] && x[1] != amariColor[1]);
		
		var lastOther = otherExist ? l.Where(x => x[1] != amariColor[0] && x[1] != amariColor[1]).Select(x => x[0]).Min() : long.MaxValue / 2;
		
		var smallerOtherColorDiff = new List<long>();
		var biggerOtherColorDiff = new List<long>();
		
		var minDiff = Math.Abs(lastBigger - lastSmaller);
		
		foreach (var i in l.OrderBy(x => x[0]))
		{
			var point = i[0];
			var color = i[1];
			
			if (color == amariColor[0])
			{
				lastSmaller = point;
				
				smallerOtherColorDiff.Add(Math.Abs(lastSmaller - lastOther));
			}
			else if (color == amariColor[1])
			{
				lastBigger = point;
				
				biggerOtherColorDiff.Add(Math.Abs(lastBigger - lastOther));
			}
			else
			{
				if (otherExist)
			...
			if (color == amariColor[0] || color == amariColor[1])
			{
				var diff = Math.Abs(lastSmaller - lastBigger);

				minDiff = Math.Min(minDiff, diff);
			}
		}
		
		var useOther = smallerOtherColorDiff.Min() + biggerOtherColorDiff.Min();

		Wl(Math.Min(minDiff, useOther));
	}
