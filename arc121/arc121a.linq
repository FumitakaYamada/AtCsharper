...
static class Program
{
	static void Main()
	{
		...
		{
			var array = inputter.GetNext().Split().Select(ToLong).Select(x => x + 1000000000).ToArray();
		...
		var increase = l.OrderBy(x => x[0]);

		var decrease = l.OrderBy(x => x[1]);

		var checking = new List<long[]>();
		
		var amount = 5;

		checking.AddRange(increase.Take(amount));
		checking.AddRange(increase.Skip(n - amount).Take(amount));
		checking.AddRange(decrease.Take(amount));
		checking.AddRange(decrease.Skip(n - amount).Take(amount));

		var diffList = new List<long>();
		
		var newChecking = new List<long[]>();
		var usedIds = new List<long>();
		
		foreach (var c in checking)
		{
			if (usedIds.Contains(c[2]))
			{
				continue;
			}
		...
		var ncc = newChecking.Count();
		
		foreach (var i in Ie(ncc))
		{
			for (var j = i + 1; j < ncc; j++)
			{
				var ii = newChecking[i];
				var jj = newChecking[j];
				
				if (ii[2] == jj[2]) continue;
				
				var diff = Math.Max(Math.Abs(ii[0] - jj[0]), Math.Abs(ii[1] - jj[1]));
				diffList.Add(diff);
			}
		}
		
		var second = diffList.OrderBy(x => x).ToArray()[diffList.Count() - 2];
