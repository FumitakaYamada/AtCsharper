...
static class Program
{
	static void Main()
	{
		...
		var reds = new List<long>();
		var greens = new List<long>();
		var blues = new List<long>();
		...
		var oneUsed = false;
		var amari1 = new long[0];
		var amari2 = new long[0];
		...
		if (greens.Count() % 2 == 1)
		{
			if (oneUsed)
			{
				amari2 = greens.ToArray();
			}
			else
			{
				amari1 = greens.ToArray();
			}
			oneUsed = true;
		}
		...
		foreach (var i in amari1.Concat(amari2).OrderBy(x => x))
		{
			//i.Dump();
			var point = i[0];
			var color = i[1];
			
			if (amariColor[0] == color)
			{
				lastSmaller = point;
			}
			else
