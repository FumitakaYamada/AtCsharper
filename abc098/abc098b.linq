...
static class Program
{
	static void Main()
	{
		...
		var max = s.ToCharArray().Distinct().Count();
		...
		while (ac + 1 < wc)
		{
			...
			for (var i = wj; i < s.Length - wj; i++)
			{
				var first = s.Substring(0, i).ToCharArray().Distinct().ToArray();
				var last = s.Substring(i, s.Length - i).ToCharArray().Distinct().ToArray();

				if (first.Count(x => last.Contains(x)) >= wj &&
					last.Count(x => first.Contains(x)) >= wj)
				{
					flag = true;
					break;
				}
