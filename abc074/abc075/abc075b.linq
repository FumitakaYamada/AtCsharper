...
static class Program
{
	...
	static void Main()
	{
		...
		{
			map.Add(inputter.GetNext().ToCharArray());
		}
		...
		bool Check(Point p)
		{
			...
			{
				return map[(int)p.X][(int)p.Y].Equals('#');
			}

			return false;
		}

		int GetNumber(Point p)
		{
			...
			return points.Count(x => Check(x));
		}
		
		foreach (var x in Ie(0, h))
		{
			var line = new List<char>();
			foreach (var y in Ie(0, w))
			{
				var p = new Point(x, y);
				if (map[(int)p.X][(int)p.Y].Equals('.'))
				{
					line.Add(GetNumber(p).ToString().ToCharArray()[0]);
					continue;
				}
				
				line.Add('#');
			}
			Wl(new String(line.ToArray()));
		}
	}
