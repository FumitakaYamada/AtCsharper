...
static class Program
{
	static void Main()
	{
		...
		var l = new List<(int b, int c)>();
		
		foreach (var i in Ie(0, m))
		{
			var z = inputter.GetNext().Split().Select(ToInt).ToArray();
			l.Add((z[0], z[1]));
		}
		
		var pq = a.ToPriorityQueue(false);
		
		var nl = new List<(int b, int c)>();
		
		var sum = 0;
		foreach (var i in l.OrderByDescending(x => x.c))
		{
			if (sum > n)
			{
				break;
		...
		l = nl.OrderBy(x => x.c).ToList();

		foreach (var ll in l)
		{
			foreach (var i in Ie(0, ll.b))
			{
				var num = pq.Dequeue();
				pq.Enqueue(num > ll.c ? num : ll.c);
			}
		}
