...
static class Program
{
	static void Main()
	{
		...
		n = inputs.Count() > 8 ? 8 : n;
		inputs = inputs.Take(8).ToArray();
		...
		void dfs()
		{
			...
			if (l.Count() == n)
			{
				...
				if (a.Sum() % 200 == b.Sum() % 200)
				{
					a = a.OrderBy(x => x).ToArray();
					b = b.OrderBy(x => x).ToArray();

					Wl("Yes");
					Wl(a.Count() + " " + string.Join(' ', l.Select((x, i) => new { x = x, i = i}).Where(x => x.x == 0).Select(x => (x.i + 1).ToString()).ToArray()));
					Wl(b.Count() + " " + string.Join(' ', l.Select((x, i) => new { x = x, i = i}).Where(x => x.x == 1).Select(x => (x.i + 1).ToString()).ToArray()));
					done = true;
				}
				return;
			}
