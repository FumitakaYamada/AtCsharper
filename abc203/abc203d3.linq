...
static class Program
{
	static void Main()
	{
		...
		// key point, value score
		var dic = new Dictionary<int, int>();

		// key point, value scores
		...
		if (false)
		{
			...
			{
				var arr = inputter.GetNext().Split().Select(ToInt).ToArray();
				l.Add(arr);
		...
		else
		{
			...
			k = 400;
			var iterat = 1;
			l = Ie(800).Select(x => Ie(x * 800, 800).Select(x => x).ToArray()).ToList();

			foreach (var i in Ie(n))
			{
				foreach (var j in Ie(n))
				{
					dic.Add(i * n + j, iterat++);
					dic2.Add(i * n + j, 0);
				}
			}
			//
		...
		var centerNum = (int)(Math.Floor(Math.Pow(k, 2) / 2) + 1);
		
		var indexFromSmall = (int)Math.Pow(k, 2) - centerNum;
		...
		long dp(int x, int y)
		{
			...
			foreach (var i in Ie(x, k))
			{
				foreach (var j in Ie(y, k))
				{
					cache.Add(l[i][j]);
				}
			}
			var oa = cache.OrderByDescending(x => x).ToArray();
			return oa[centerNum - 1];
		}

		foreach (var p in dic.OrderBy(x => x.Value))
		{
			var point = p.Key;
			var px = point / n;
			var py = point % n;

			var minx = Math.Max(0, px - k + 1);
			var maxx = Math.Min(n - 1, px + k - 1);
			var miny = Math.Max(0, py - k + 1);
			var maxy = Math.Min(n - 1, py + k - 1);
			
			var xc = (px - minx + 1);
			var yc = (py - miny + 1);

			foreach (var i in Ie(minx, xc))
			{
				foreach (var j in Ie(miny, yc))
				{
					var xp = i;
					var yp = j;
