...
static class Program
{
	static void Main()
	{
		...
		foreach (var i in Ie((int)n))
		{
			l.Add(inputter.GetNext().Split().Select(ToLong).ToArray());
		}
		
		// debug
		//n = 200000L;
		//k = 1000100100L;
		//l = Ie(1, (int)n).Select(x => new long[] { x + k - 1, 1 }).ToList();
		//
		
		var dic = l.GroupBy(x => x[0]).ToDictionary(x => x.Key, x => x.Select(y => y[1]).Sum());
//		
//		var ac = 0L;
//		var leftMoney = k;
//
...
//			var current = ac;
//			ac += leftMoney;
//			leftMoney = dic.Where(x => current < x.Key && x.Key <= (current + leftMoney)).Select(x => x.Value).Sum();
//			//leftMoney = Enumerable.Range(current, leftMoney).Select(x => x[1]).Sum();
//		}

		var ac = k;
		
		foreach (var f in dic.OrderBy(x => x.Key))
		{
			if (f.Key <= ac)
			{
				ac += f.Value;
